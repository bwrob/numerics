...
from utils import debugging, RootFindingData, animate
...
def root_newton(debug, f, start_point, epsilon, h=0.000001):
    ...
    while i < max_iter:
        ...
        if abs(value) < epsilon:
            return RootFindingData(value=current_point, iteration_points=points, iteration_no=i)
        if debug:
...
def root_secant(debug, f, point_zero, point_one, epsilon):
    ...
    while i < max_iter:
        ...
        if abs(value_one) < epsilon:
            return RootFindingData(value=point_one, iteration_points=points, iteration_no=i)
        if debug:
